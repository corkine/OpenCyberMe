...
[1.4.4 2022-3-30]
重新实现了响应式日记前端界面，提供了每日评分系统。
[1.4.5 2022-4-05]
Todo没有显示 0，有最少显示5% 哪怕一个也没完成。
运动数据改为平衡卡路里而非运动卡路里，更新 hint 信息。
提供周视图，包括每天日记，平衡和 Blue 情况展示。
更新进度条，使得背景更加融合。
[1.4.6 2022-4-06]
每天只要达到自己满意分数即合并到周视图，而非一定要 100%。
周视图图表和进度条包含现在所有的项目，包括 clean, todo, blue, fitness, today-score。
TODO 计算方法更新：每天至少有一项 TODO 任务且全部完成。如果有 TODO 但未开始，圆环设定为 0.1，如果没有 TODO，圆环直接为 0 而非 1。
实现了最长 clean 和最长 blue 的数据记录和获取，每次重置后最高纪录不再重置。
[1.4.7 2022-4-11]
日记设置最大宽度，提升用户体验。
修复了物品过滤器和默认 PlaceId 的问题。
[1.4.8 2022-4-12]
日记和物品支持 filter 记忆。
日记改为双列布局。
[1.4.9 2022-4-26]
实现了全局剪贴板监听自动上传图片到图床的功能，给予操作反馈且允许关闭此功能。
